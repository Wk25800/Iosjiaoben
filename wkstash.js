// 2026/06/16 - Stash 适配版（基于 Loon 版改写 + 通知精简修复）
/*
WeTalk Stash version
顶部仅作说明，Stash 配置见配套 .stoverride 文件，与此处注释无关。
*/
...
const SCRIPT_VERSION = '2026.06.16-stash.2';
...
function notify(title, body) {
  ...
  // 完整内容始终写入日志，方便排查
  log(`${subtitle}${fullContent ? '\n' + fullContent : ''}`);

  let content = fullContent;
  if (content.length > NOTIFY_BODY_MAX) {
    content = content.slice(0, NOTIFY_BODY_MAX) + '…\n📋 完整结果见脚本日志';
  }
...
function startWatchdog(ms, label) {
  ...
  watchdogTimer = trackTimer(function() {
    ...
    notify('任务超时结束', `⏰ ${label} 超过 ${Math.round(ms / 1000)} 秒，已主动结束`);
    done({});
  }, ms);
}
...
function captureAccount() {
  ...
  if (!email) {
    ...
    notify('抓取失败', '请求里未取到 email 参数，请确认已登录后再触发抓包');
    done({});
    return;
  }
  ...
  notify(existed ? '账号参数已更新' : '新账号已入库', `${email} 当前账号总数: ${total}`);
...
function runTask() {
  ...
  if (!ids.length) {
    notify('未抓到任何账号', '请先打开 WeTalk 触发抓包，抓到后再运行签到任务');
    done();
    return;
  }
  ...
  chain.then(function() {
    // 完整结果写入日志
    log('full results:\n' + results.join('\n🎀 --- 🎀\n'));
    // 通知体仅给精简汇总，避免长文本被系统静默丢弃
    const names = ids.map(function(id) {
      const acc = store.accounts[id];
      return maskAccount(acc.alias || acc.email || acc.id);
    }).join('、');
    notify(`全部完成 (${total}个账号)`, `${names} 已处理，详情见脚本日志`);
    done();
  }).catch(function(err) {
    log('full results on error:\n' + results.join('\n🎀 --- 🎀\n'));
    notify('任务异常', `${err.error || String(err)}，详情见脚本日志`);
    done();
  });
}
