...
script-providers:
  PingMe:
    url: https://raw.githubusercontent.com/ZenmoFeiShi/Qx/refs/heads/main/PingMe_Stash.js
    interval: 86400
...
const SECRET = '0fOiukQq7jXZV2GRi9LG1O';
const MAX_VIDEO = 5;
...
// Stash 任务运行逻辑
  ...
  if (!raw) {
    ...
    try {
      ...
      $notification.post(scriptName, '⚠️ 参数格式错误', '重新抓参');
      $done();
    }
    ...
    function fetchApi(path) {
      return new Promise((resolve, reject) => {
        $httpClient.get({
          ...
          if (err) reject(err);
          else resolve({ status: res.status, body });
        });
      });
    }
    ...
    fetchApi('queryBalanceAndBonus').then(res => {
      ...
      $notification.post(scriptName, '❌ 任务失败', err.message || err);
      $done();
    });
  }
}
