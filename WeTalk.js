...
function migrateStore(store) {
  ...
  if (migrated) { store.accounts = newAccounts; store.order = newOrder; }
  return store;
}
    ...
    $httpClient.get({ url, headers }, (error, response, body) => {
      if (error) {
        ...
        if (retry > 0 && /SSL|timeout|timed out|reset|connection|network|stream closed|closed|EOF/i.test(m)) {
          setTimeout(() => fetchApi(url, headers, retry - 1).then(resolve).catch(reject), 1200);
        } else {
          reject({ error: m });
        }
      } else {
...
function runAccount(acc, index, total) {
  ...
  function req(path, useFakeId) {
    const overrideId = useFakeId ? fakeDeviceId : null;
    return fetchApi(buildUrl(path, acc.capture, overrideId), headers);
  }
  ...
  return req('queryBalanceAndBonus').then(res => {
    try {
      const d = JSON.parse(res.body);
  ...
  }).then(() => req('queryBalanceAndBonus')).then(res => {
    try {
      const d = JSON.parse(res.body);
