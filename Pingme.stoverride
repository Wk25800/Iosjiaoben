...
(function() {
    ...
    // 任务模式（手动/定时触发）
        ...
        function doVideoLoop(count, done) {
            let i = 0;
            function next() {
                if (i >= count) return done();
                setTimeout(() => {
                    i++;
                    fetchApi('videoBonus', (err, resp, data) => {
                        if (err) {
                            msgs.push(`❌ 视频${i}：请求失败`);
                        } else {
                            try {
        ...
        fetchApi('queryBalanceAndBonus', (err, resp, data) => {
            if (err) {
                msgs.push('❌ 查询余额：请求失败');
                runSign();
                return;
        ...
        function runSign() {
            fetchApi('checkIn', (err, resp, data) => {
                if (err) {
                    msgs.push('❌ 签到：请求失败');
                    runVideo();
                    return;
